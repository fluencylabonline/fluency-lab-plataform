...
export default async function PrivacyPage({ params }: Props) {
  ...
  const privacyPolicy = await import(`@/modules/contract/privacy-policy.${locale}.json`)
    .then((m) => m.default)
    .catch(() => import(`@/modules/contract/privacy-policy.pt.json`).then((m) => m.default));
          ...
          <div className="card p-6 md:p-12 border-none bg-transparent dark:bg-transparent hover:bg-transparent dark:hover:bg-transparent">
            <header className="mb-12 text-center md:text-left">
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 text-sm text-muted-foreground">
                <p>{t("version")}: {privacyPolicy.version}</p>
                <span className="hidden md:inline text-muted-foreground/30">•</span>
                <p>{t("effectiveDate")}: {privacyPolicy.effectiveDate}</p>
              </div>
            </header>

            <div className="space-y-12 md:space-y-20">
              {privacyPolicy.sections.map((section: { title: string; content: string }, index: number) => (
