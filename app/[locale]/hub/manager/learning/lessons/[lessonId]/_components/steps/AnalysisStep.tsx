...
export function AnalysisStep({ 
    ...
    const contentMatches = step === 7 && contentText?.trim() === transcriptionText?.trim();
    const shouldSkip = step === 7 && contentMatches && hasExistingData;

    const [stepStatus, setStepStatus] = useState<"idle" | "analyzing" | "completed" | "error">(
        isStepDone || lessonStatus === "ready" || shouldSkip ? "completed" : "idle"
    );
    const [progress, setProgress] = useState(hasExistingData || shouldSkip ? 100 : 0);
    ...
    // Auto-advance if shouldSkip is true
    useEffect(() => {
        if (shouldSkip && stepStatus === "completed") {
            onComplete();
        }
    }, [shouldSkip, stepStatus, onComplete]);

    const getDefaultTask = () => {
        if (shouldSkip) return t("already_synced") || "Content identical to transcription. Already synced.";
        if (hasExistingData) return t("analysis_done") || "Analysis complete!";
        return "";
    };
    const [currentTask, setCurrentTask] = useState(getDefaultTask());

    // Simulate progress while analyzing
    useEffect(() => {
        let interval: NodeJS.Timeout;
        if (stepStatus === "analyzing") {
    ...
    const startAnalysis = async () => {
        try {
            ...
            // 1. Update lesson status to analyzing
            await updateLessonAction({ id: lessonId, status: "analyzing" });
            ...
            <div className="text-center space-y-4 mb-4">
                <div className="relative inline-block">
                    <div className="w-20 h-20 bg-primary/10 rounded-3xl flex items-center justify-center mx-auto relative z-10">
                        <Brain className={cn("w-10 h-10 text-primary", stepStatus === "analyzing" && "animate-pulse")} />
                    </div>
                ...
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">
                        {step === 4 ? (t("transcription_analysis_title") || "Transcription Analysis") : (t("lesson_analysis_title") || "Lesson Content Extraction")}
                    </h2>
                    <p className="text-muted-foreground mt-2 max-w-lg mx-auto text-base">
                        {step === 4 
                            ? (t("transcription_analysis_desc") || "Identifying vocabulary and structures from the video transcription.") 
                            : (t("lesson_analysis_desc") || "Extracting learning items from the refined lesson text and merging with transcription items.")}
                    </p>
                </div>
            </div>
            ...
            {stepStatus === "idle" ? (
                ...
                <div className="space-y-10">
                    ...
                    <div className="bg-card rounded-md p-8 border border-border shadow-sm space-y-6 relative overflow-hidden">
                        {stepStatus === "analyzing" && (
                            <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-primary/10 via-primary/40 to-primary/10 animate-shimmer" />
                        )}
                        ...
                        <div className="flex items-center justify-between relative z-10">
                            <div className="flex items-center gap-4">
                                <div className={cn(
                                    ...
                                    stepStatus === "completed" ? "bg-emerald-500/10 text-emerald-500" : "bg-primary/10 text-primary"
                                )}>
                                    {stepStatus === "analyzing" ? <Loader2 className="w-6 h-6 animate-spin" /> : <CheckCircle2 className="w-6 h-6" />}
                                </div>
                                <div>
                                    <h3 className="font-bold text-xl">
                                        {stepStatus === "analyzing" ? t("ai_is_working") : t("analysis_done")}
                                    </h3>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {step === 7 && (
                                            <Button variant="outline" size="sm" onClick={startAnalysis}>
                                                ...
                                                {t("re_analyze") || "Sync with Content"}
                                            </Button>
                                        )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-card rounded-md border border-border overflow-hidden shadow-sm flex flex-col min-h-[400px]">
                            ...
                            <div className="p-6 flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                                {stepStatus === "analyzing" && vocabulary.length === 0 ? (
                                    // Skeletons
                                    ...
                                    vocabulary.map((item, i) => (
                                        <div key={i} className="group p-4 rounded-2xl bg-muted/30 border border-border/50 hover:border-primary/30 hover:bg-primary/2 transition-all animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${i * 50}ms` }}>
                        ...
                        <div className="bg-card rounded-md border border-border overflow-hidden shadow-sm flex flex-col min-h-[400px]">
                            ...
                            <div className="p-6 flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                                {stepStatus === "analyzing" && structures.length === 0 ? (
                                    // Skeletons
                                    Array.from({ length: 5 }).map((_, i) => (
                                        <div key={i} className="h-16 w-full rounded-2xl bg-muted/40 animate-pulse border border-border/50" />
                                    ))
                                    ...
                                    structures.map((item, i) => (
                                        <div key={i} className="group p-4 rounded-2xl bg-muted/30 border border-border/50 hover:border-amber-500/30 hover:bg-amber-500/2 transition-all animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${i * 50}ms` }}>
                                            <div className="flex items-start justify-between gap-2">
